...
namespace ofxAzureKinect
{
	DeviceSettings::DeviceSettings(int idx)
		...
		, colorFormat(K4A_IMAGE_FORMAT_COLOR_MJPG)
	...
	void Device::updateCameras(ofEventArgs& args)
	{
		...
		if (this->bUpdateColor)
		{
			...
			{	
				...
				//ofLogWarning(__FUNCTION__) << colorImg.get_format();
				//ofLogWarning(__FUNCTION__) << colorImg.get_width_pixels();
				//ofLogWarning(__FUNCTION__) << colorImg.get_height_pixels();
		...
		if (this->bUpdateBodies)
		{
			...
			else
			{
				...
				{	
					...
					// Expose timeStamp of bodyFrame
					this->timeStamp = k4abt_frame_get_device_timestamp_usec(bodyFrame);
	...
	const uint64_t Device::getTimeStamp() const
	{
		return this->timeStamp;
	}

	const k4a::image Device::getImage() const
	{
